        ...
        return (
          <>
            ...
            <SectionCard style={{ background: 'var(--bg-0)', borderColor: 'var(--line-0)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ['Дашборд судьи: обзор, фокус дня и рабочий стол с данными из дел', 'var(--ac)'],
                  ['Печать на документах теперь справа внизу; ползунки отступа работают от краёв страницы', 'var(--blue)'],
                  ['Коллегия: подписи и печати судей идут друг под другом, каждая со своей печатью', 'var(--green)'],
                  ['Закрытые дела можно вернуть в работу через меню дела', 'var(--amber)'],
                  ['Ордер: единый документ — сначала авторизация, затем субъект (Arrest and search)', 'var(--purple)'],
                  ['Вкладка «Участники» переработана: коллегия судей, карточки и подсказки', 'var(--red)'],
                ].map(([text, color]) => (
